...
Maintainers assign versions using `YYYY.MM.NNN` (CalVer): four-digit year, two-digit month (01–12) and a three-digit monthly sequence (001–999). Store the value as a quoted string so leading zeros survive. Current metadata is `2026.09.010`; use `v2026.09.010` for the GitHub tag/release and `blendsmith-v2026.09.010.zip` for the archive. The initial sequence 010 is a chosen starting label, not evidence of nine public releases. Increment the sequence for another release in the same month; start at 001 in a new month. If the sequence range is exhausted, agree a format revision before publishing.
